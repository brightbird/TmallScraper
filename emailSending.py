...
class sendEmail(object):
    ...
    def constructEmail(self, member):
        ...
        self.main_msg['subject'] = 'Scraper log by {} list from Tmall'.format(member)
...
if __name__ == '__main__':
    begin = time.time()
    file_name = '/home/ubuntu/TmallData_' +str(time.strftime('%Y-%m-%d',time.localtime(time.time()))) + '.rar'
    Email = sendEmail(to_addr='413523287@qq.com', scraper_log='/home/ubuntu/log/scraper.log', attch=file_name)
    Email.getLogContent()
    Email.constructEmail('test')
    Email.sendEmail()
    end = time.time()
